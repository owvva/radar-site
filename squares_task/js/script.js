...
let square_colors = ['#FF0000', '#FFFF00', '#00FF00', '#00FFFF', '#0000FF', '#FF007F','#FF8000'];
...
var square_raz = 110;
var square_width = square_raz + 20;
...
for (var j = 0; j < window_m; j++) {
  window_raz[j] = Math.floor(document.documentElement.clientWidth / window_m * j + 20);
}
...
function draw_main_ram(rect_main) {
  ...
  ctx.strokeRect(0, 0, square_raz + 20, square_raz + 20);
}
...
function draw(rect_new, width_ = square_raz, otsup = 0) {
  ...
  for (i = 0; i < raz_sqv; i += 1) {
    for (j = 0; j < raz_sqv; j+=1) {         
      ctx.fillStyle = square_colors[randomInteger(0, level + 2)];
      ctx.fillRect(i * (square_raz / level_raz[level]) + otsup, j * (square_raz / level_raz[level]) + otsup, (square_raz / level_raz[level]), (square_raz / level_raz[level]));
    }
  }
  for (i = 0; i <= raz_sqv; i += 1) {
    for (j = 0; j <= raz_sqv; j += 1) {
      if (i == 0) {
        ctx.strokeStyle = '#000000';
        ctx.beginPath();
        ctx.moveTo( otsup, j * (square_raz / level_raz[level]) + otsup);
        ctx.lineTo( otsup + square_raz, j * (square_raz / level_raz[level]) + otsup);
        ctx.closePath();
        ctx.stroke();
      }
      if (j == 0) {
        ctx.strokeStyle = '#000000';
        ctx.beginPath();
        ctx.moveTo(i * (square_raz / level_raz[level]) + otsup, otsup);
        ctx.lineTo(i * (square_raz / level_raz[level]) + otsup , square_raz + otsup);
        ctx.closePath();
        ctx.stroke();
      }
    }
...
function cloneCanvas(oldCanvas) {
  ...
  context.drawImage(oldCanvas, 10, 10, square_raz, square_raz, 0, 0, square_raz, square_raz);
  context.lineWidth = 3;
...
window.onload = function() {
  ...
  for (let i = 0; i<square_count[level] - 1; i += 1) {
    ...
    draw(new_rect, square_raz, 0);

    var x = randomInteger(0,11) * 30;
    var rand_int_wind = randomInteger(0, window_count.length - 1);
      
    new_rect.style.left = window_raz[window_count[rand_int_wind] % 10]+'px';
  ...
  rect_answer.style.left = window_raz[window_count[rand_int_wind] % 10]+'px';
